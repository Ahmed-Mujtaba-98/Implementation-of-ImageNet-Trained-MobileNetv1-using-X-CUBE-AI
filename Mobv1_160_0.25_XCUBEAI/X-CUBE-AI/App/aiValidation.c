...
static ai_u32 aiOnExecNode_cb(const ai_handle cookie,
  ...
  if (flags & AI_OBSERVER_PRE_EVT) {
    ...
    AI_FOR_EACH_TENSOR_LIST_DO(i, t, tl) {
      ...
      /* temporary fix before to update msg i/f for shape dim > 4 */
      ...
      ai_buffer buffer =
          AI_BUFFER_INIT(AI_FLAG_NONE,                          /* flags */
            fmt,                                                /* format */
            AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, (d_) * (e_), (ch_), (w_), (h_)), /* shape */
            (d_) * (e_) * (h_) * (w_) * AI_PAD_CHANNELS(fmt, ch_),            /* size */
            NULL,                                               /* meta info */
